...
export default function RootLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    setIsAuth(!!token)
  }, [pathname]) // Перепроверяем при смене пути

  // Чтобы избежать ошибки гидратации, дожидаемся монтирования
  if (!mounted) return <html lang="ru"><body></body></html>

  // Считаем страницу "авторизационной", если путь /login или если нет токена на главной
    ...
    <html lang="ru" suppressHydrationWarning> 
      <body className={`${geistSans.variable} ${geistMono.variable} font-sans antialiased`}>
        <ThemeProvider attribute="class" defaultTheme="dark" enableSystem>
          ...
          {/* Если НЕ показываем сайдбар, убираем все ограничивающие флекс-контейнеры */}
          {!showSidebar ? (
