...
interface AddTaskDialogProps {
  ...
  onAddTask: (task: any) => void // Используем any для гибкости с бэкендом
  nextId: number
  defaultStatus?: string // ИСПРАВЛЕНИЕ 1: Добавили пропс, чтобы TS не ругался в kanban-board.tsx
}
...
export function AddTaskDialog({
  ...
  // ИСПРАВЛЕНИЕ 2: При каждом открытии окна сбрасываем поля и ставим статус колонки
  useEffect(() => {
    if (open) {
      setTitle("")
      setDescription("")
      setPriority("medium")
      setDeadline("")
      // Если передали статус колонки (например, "done"), ставим его в селект
  ...
  const handleSubmit = () => {
    ...
    // Отправляем данные. Поле status теперь точно берется из стейта формы
    onAddTask({
      title: title.trim(),
