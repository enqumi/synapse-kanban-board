  ...
  const handleAddTask = async (taskData: any) => {
    try {
      await createTask({
        ...
        // ГЛАВНОЕ ИЗМЕНЕНИЕ: 
        // Сначала пробуем взять статус из самого диалога (taskData.status).
        // Если его там нет, берем статус колонки (activeColumnStatus).
  ...
  return (
    <div className="flex flex-1 flex-col overflow-hidden">
      ...
      <AddTaskDialog
        ...
        // Передаем текущий статус колонки, чтобы диалог знал, что показать по умолчанию
